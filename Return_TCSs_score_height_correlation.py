...
path_to_res_score_files="F:\TopoIV_Topo-Seq\GCSs_analysis\Score_tracks\E_coli_w3110_G_Mu_genome_score.wig"
...
path_to_GCSs_sets_with_score="F:\TopoIV_Topo-Seq\GCSs_analysis\GCSs_sets_score\\"
if not os.path.exists(path_to_GCSs_sets_with_score):
    os.makedirs(path_to_GCSs_sets_with_score)
Outpath_to_GCSs_files={'Cfx': path_to_GCSs_sets_with_score + "Cfx_trusted_TCSs_h_s.txt"}
...
def Plot_N3E_score(HS_dict, pearson_cor, plot_path):
    ...
    plot0.annotate('Pearson correlation='+str(round(pearson_cor[0],4)), xy=(0.7, 0.9), xycoords='axes fraction',  weight="bold", size=18)
    ...
    plt.tight_layout()
    plt.savefig(plot_path + "TCSs_N3E_score_correlations.png", dpi=400, figsize=(15, 15)) 
    plt.close()
    return
...
def Plot_N3E_score_distribution(HS_dict, score_ar, plot_path):
    ...
    plt.tight_layout()
    plt.savefig(plot_path + "TCSs_N3E_score_distributions.png", dpi=400, figsize=(15, 15)) 
    plt.close()    
    return
...
def Wrapper(path_s, input_dict, output_dict, plot_path):
    ...
    Plot_N3E_score(Sorted_for_plot_dict, pearson_cor, plot_path)
