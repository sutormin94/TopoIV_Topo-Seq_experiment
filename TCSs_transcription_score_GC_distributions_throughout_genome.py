...
#Input data - GCSs, TAB.
path_to_GCSs_files={'Cfx TopoIV': "F:\TopoIV_Topo-Seq\GCSs_analysis\GCSs_sets_score\Cfx_trusted_TCSs_h_s.txt",
                    'Cfx Gyrase' : "C:\Sutor\science\DNA-gyrase\Results\GCSs_sets_and_motifs\GCSs_sets_score\Cfx_10mkM_trusted_GCSs_h_s.txt"}                    

#Input data - score, WIG.
Score_data_path="F:\TopoIV_Topo-Seq\GCSs_analysis\Score_tracks\E_coli_w3110_G_Mu_genome_score.wig"
...
#Output data: plot.
Plot_path_out="F:\TopoIV_Topo-Seq\GCSs_analysis\GCSs_num_score_GC_transcription_distrib_thr_genome_with_MDs\\"
...
def Plot_the_distribution(GSCs_data, Non_GCSs_data, bins, genome_len, path_out):
    ...
    Y_labels=['Cfx TCSs', 'Cfx GCSs', 'Score', 'GC%', 'Transcription\nlevel']
    yter=1592477
    yori=3711828
    #GCSs data plotting.
    fig, plot_names=plt.subplots(len(GSCs_data)+len(Non_GCSs_data),1,figsize=(11,(2*len(Y_labels))+1), dpi=100)
    i=0
    ...
    for key, value in GSCs_data.items():
        ...
        conf_interval=[binom.interval(0.999, len(value), 1/(len(bins)-1))[0], binom.interval(0.999, len(value), 1/(len(bins)-1))[1]]
        ...
        Histo_comp_dict[key]=plot_names[i].hist(value, bins, facecolor=colors[i], alpha=0.7, linewidth=10/(len(bins)-1), edgecolor='black') #Plot histo and save computed histogramm data (bins and values)
    ...
    for key, value in Non_GCSs_data.items():
        ...
        plot_names[i].tick_params(axis='x', which='major', labelsize=19)
        plot_names[i].locator_params(axis='y', nbins=6)
        plot_names[i].bar(position, value, bin_width, color=colors[i], linewidth=10/len(position), edgecolor='black', align='edge')
        plot_names[i].tick_params(axis='y', which='major', pad=7, labelsize=15)
        plot_names[i].set_ylabel(Y_labels[i], size=22, labelpad=8, rotation=90) 
        i+=1
    plt.tight_layout()
    fig.savefig(path_out+"TCSs_num_score_GC133_transcription_distrib_thr_genome_"+str(len(position))+"_bins.png", figsize=(11,(2*len(Y_labels))+1), dpi=400)
    plt.close()
    
    #GCSs data plotting for Cfx, Micro, and Oxo only.
    GSCs_data_main={'Cfx TopoIV': GSCs_data['Cfx TopoIV'], 'Cfx Gyrase': GSCs_data['Cfx Gyrase']}
    Y_labels_main=['Cfx TCSs', 'Cfx GCSs']
    colors_main=['#7FCE79', '#ff878b', '#8991ff']
    fig, plot_names=plt.subplots(len(GSCs_data_main),1,figsize=(11,(2*len(Y_labels_main))+1), dpi=100)
    if len(GSCs_data_main)>1:
        i=0
        for key, value in GSCs_data_main.items():
            ...
            plot_names[i].set_xticklabels(xticknames1)
            plt.setp(plot_names[i].set_xticklabels(xticknames1), rotation=0, fontsize=14)
            conf_interval=[binom.interval(0.999, len(value), 1/len(position))[0], binom.interval(0.999, len(value), 1/len(position))[1]]
            plot_names[i].set_yticks(conf_interval, minor=True)
            plot_names[i].yaxis.grid(True, which='minor', linewidth=0.4, linestyle='--', color='black')
            plot_names[i].fill_between(bins, conf_interval[0], conf_interval[1], facecolor='grey', alpha=0.3)
            plot_names[i].locator_params(axis='y', nbins=6)
            plot_names[i].tick_params(axis='x', which='major', labelsize=19)
            plot_names[i].hist(value, bins, facecolor=colors_main[i], alpha=0.7, linewidth=10/len(position), edgecolor='black') #Plot histo and save computed histogramm data (bins and values)
            plot_names[i].tick_params(axis='y', which='major', pad=7, labelsize=15)
            plot_names[i].set_ylabel(Y_labels_main[i], size=22, labelpad=8, rotation=90)
            i+=1  
    elif len(GSCs_data_main)==1:
        i=0
        for key, value in GSCs_data_main.items():
            plot_names.set_xlim(0, genome_len)
            plot_names.set_xticks(ticks1, minor=False)
            plot_names.set_xticks([yter, yori], minor=True)
            plot_names.set_xticklabels(xticknames1)
            plt.setp(plot_names.set_xticklabels(xticknames1), rotation=0, fontsize=14)
            conf_interval=[binom.interval(0.999, len(value), 1/len(position))[0], binom.interval(0.999, len(value), 1/len(position))[1]]
            plot_names.set_yticks(conf_interval, minor=True)
            plot_names.yaxis.grid(True, which='minor', linewidth=0.4, linestyle='--', color='black')
            plot_names.fill_between(bins, conf_interval[0], conf_interval[1], facecolor='grey', alpha=0.3)
            plot_names.locator_params(axis='y', nbins=6)
            plot_names.tick_params(axis='x', which='major', labelsize=19)
            plot_names.hist(value, bins, facecolor=colors_main[i], alpha=0.7, linewidth=10/len(position), edgecolor='black') #Plot histo and save computed histogramm data (bins and values)
            plot_names.tick_params(axis='y', which='major', pad=7, labelsize=15)
            plot_names.set_ylabel(Y_labels_main[i], size=22, labelpad=8, rotation=90)  
            i+=1
    plt.tight_layout()
    fig.savefig(path_out+"TCSs_number_Cfx_distrib_thr_genome_"+str(len(position))+"_bins.png", figsize=(11,(2*len(Y_labels_main))+1), dpi=400) 
    plt.close()
...
def Plot_NAPs_distribution(Some_data, bins, path_out, genome_len, binning_type):
    ...
    #Data plotting.
    fig, plot_names=plt.subplots(len(Some_data),1,figsize=(11,(2*len(plot_names))+1), dpi=100)
    i=0
    ...
    for key, value in Some_data.items():
        ...
        plot_names[i].tick_params(axis='x', which='major', labelsize=19)
        plot_names[i].locator_params(axis='y', nbins=6)
        plot_names[i].bar(position, value, bin_width, color=colors[i], linewidth=10/len(value), edgecolor='black', align='edge')
        plot_names[i].tick_params(axis='y', which='major', pad=7, labelsize=15)
        plot_names[i].set_ylabel(key, size=22, labelpad=8, rotation=90) 
        i+=1
    plt.tight_layout()
    fig.savefig(path_out, figsize=(11,(2*len(plot_names))+1), dpi=400)  
    plt.close()
    return 
...
def Plot_the_distribution_MDs(GSCs_data, Non_GCSs_data, bins, MDs_lengths, genome_len, path_out):
    ...
    #GCSs data plotting.
    fig, plot_names=plt.subplots(len(GCSs_data_bared)+len(Non_GCSs_data),1,figsize=(11,(2*len(Y_labels))+1), dpi=100)
    print(bins)
    position=[0]+bins[1:]
    print(position)
    bin_width=[]
    ...
    for key, value in Non_GCSs_data.items():
        ...
        i+=1
    plt.tight_layout()
    fig.savefig(path_out+"TCSs_num_score_GC133_transcription_distrib_thr_genome.png", figsize=(11,(2*len(Y_labels))+1), dpi=400) 
    plt.close()
    
    #GCSs data plotting for Cfx, Micro, and Oxo only.
    GSCs_data_main={'Cfx TopoIV': GCSs_data_bared['Cfx TopoIV'], 'Cfx Gyrase': GCSs_data_bared['Cfx Gyrase']}
    Y_labels_main=['Cfx TCSs', 'Cfx GCSs']
    colors_main=['#7FCE79', '#ff878b', '#8991ff']
    fig, plot_names=plt.subplots(len(GSCs_data_main),1,figsize=(11,(2*len(Y_labels_main))+1), dpi=100)
    if len(GSCs_data_main)>1:
        ...
        for key, value in GSCs_data_main.items():
            ...
            plot_names.locator_params(axis='y', nbins=6)
            plot_names.tick_params(axis='x', which='major', labelsize=19)
            plot_names.bar(position, value, bin_width, color=colors_main[i], linewidth=1, edgecolor='black', align='edge')
            plot_names.tick_params(axis='y', which='major', pad=7, labelsize=15)
            plot_names.set_ylabel(Y_labels_main[i], size=22, labelpad=8, rotation=90)
            i+=1           
    plt.tight_layout()
    fig.savefig(path_out+"TCSs_number_Cfx_distrib_thr_genome.png", figsize=(11,(2*len(Y_labels_main))+1), dpi=400) 
    plt.close()
...
def track_corr(GCSs_histo_comp_dict, Non_GCSs_data):
    Cfx_TCSs=np.array(GCSs_histo_comp_dict['Cfx TopoIV'][0]).tolist()
    Cfx_GCSs=np.array(GCSs_histo_comp_dict['Cfx Gyrase'][0]).tolist()
    ...
    print('Paerson correlation (Cfx TopoIV, transcription) for even bars: ' + str(pearsonr(Cfx_TCSs, Transcription)))
    print('Paerson correlation (Cfx Gyrase, transcription) for even bars: ' + str(pearsonr(Cfx_GCSs, Transcription)))
    print('Paerson correlation (Cfx TopoIV, Cfx Gyrase) for even bars: ' + str(pearsonr(Cfx_TCSs, Cfx_GCSs)))
    #print('Paerson correlation (RifCfx, transcription) for even bars: ' + str(pearsonr(RifCfx_GCSs, Transcription)))
    #print('Paerson correlation (Cfx, score) for even bars: ' + str(pearsonr(Cfx_TCSs, Score)))
...
def track_corr_MDs(GCSs_data_bared, Non_GCSs_data_MDs):
    ...
    print('Paerson correlation (Cfx TopoIV, transcription) for macrodomains: ' + str(pearsonr(GCSs_data_bared['Cfx TopoIV'], Transcription)))
    print('Paerson correlation (Cfx Gyrase, transcription) for macrodomains: ' + str(pearsonr(GCSs_data_bared['Cfx Gyrase'], Transcription)))
    print('Paerson correlation (Cfx TopoIV, Cfx Gyrase) for macrodomains: ' + str(pearsonr(GCSs_data_bared['Cfx TopoIV'], GCSs_data_bared['Cfx Gyrase'])))
...
def wrap_the_functions(input_dict, broadpeak_path, score_path, GC_path, transcription_path, NAPs_input_dict, path_out, genome_len):
    ...
    Non_GCSs_data_even=Prepare_non_GCSs_data(score_path, GC_path, transcription_path, np.linspace(0, genome_len, bins_number).tolist())
    GCSs_histo_comp_dict_even=Plot_the_distribution(GSCs_data, Non_GCSs_data_even, np.linspace(0, genome_len, bins_number).tolist(), genome_len, path_out+'Even_')
    ...
    NAPs_data_even=BroadPeak_to_bins(NAPs_input_dict, np.linspace(0, genome_len, bins_number).tolist())[1]
    Plot_NAPs_distribution(NAPs_data_even, np.linspace(0, genome_len, bins_number).tolist(), path_out+"NAPs_binding_distrib_thr_genome_even_"+str(bins_number-1)+"_bins.png", genome_len, 'even')
