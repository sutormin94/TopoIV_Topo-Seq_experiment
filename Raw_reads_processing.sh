...
Adapters='Trimmomatic-0.38/adapters/All_adapters.fa'
trimmomatic='Trimmomatic-0.38/trimmomatic-0.38.jar'
...
fastqc -t 15 -o $PWD/Fastqc_analysis/ $PWD/Raw_data/*
...
fastqc -t 15 -o $PWD/Fastqc_analysis/Trimmed_gentely/ $PWD/Trimmed_gentely/*
...
#######################
'
mkdir $PWD/SAM_trimmed/
for i in `ls -a $PWD/Trimmed_gentely/ | grep '_paired_R[1,2].fastq.gz' | sed -r "s/(.+)_paired_R[1,2]\.fastq\.gz/\1/g" | uniq | sort -d`; do 
bwa mem -t 15 $Ref_genome $PWD/Trimmed_gentely/${i}_paired_R1.fastq.gz $PWD/Trimmed_gentely/${i}_paired_R2.fastq.gz > $PWD/SAM_trimmed/$i.sam; done
...
#######################
'
for i in `ls -a $PWD/SAM_trimmed/ | grep '.sam' | sed -r "s/(.+).sam/\1/g"`; do 
samtools view -S -b $PWD/SAM_trimmed/${i}.sam > $PWD/BAM/${i}.bam ; done
